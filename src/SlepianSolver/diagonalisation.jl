...
"""
function make_concentration_operator(R::AbstractArray,K::AbstractArray,real_tapers=Val{true}())
...
"""
function compute_eigenfunctions(R::AbstractArray{Bool,D},K::AbstractArray{Bool,D},ntapers=1;imag_tol=1e-5,real_tapers=true, tol=0.0) where {D}
    _compute_eigenfunctions(R,K,ntapers,imag_tol,Val{real_tapers}(),tol)
end
function _compute_eigenfunctions(R::AbstractArray{Bool,D},K::AbstractArray{Bool,D},ntapers,imag_tol,real_tapers,tol) where {D}
