...
preallocate_output(c::CompositeLinearOperator, x) = preallocate_output(c.output, c.intermediate)
preallocate_output(::CompositeLinearOperator{LinearOperator,LinearOperator,Nothing}, x) = error("Not well specified enough to run composite operator. Please rebuild.")
...
function Base.:*(output::LinearOperator, input::LinearOperator)
    ...
    _CompositeLinearOperator(input, output, preallocate_output(input, preallocate_input(input)))
end

_CompositeLinearOperator(input::LinearOperator, output::LinearOperator, intermediate) = CompositeLinearOperator(input, output, intermediate)
function _CompositeLinearOperator(input::LinearOperator, output::CompositeLinearOperator{L1,L2,Nothing}, intermediate) where {L1,L2}
    if intermediate === nothing
        ...
        rebuilt_output = _CompositeLinearOperator(output.input, output.output, preallocate_output(output.input, intermediate)) # because intermediate is the input type of output.input
...
preallocate_output(::SupportProjection{T,D,A}, x::AbstractArray{S,D}) where {T,S,D,A} = similar(x)
...
function LinearAlgebra.mul!(y::AbstractVector{T}, v::Vec{T,D}, x::AbstractArray{T,D}) where {T,D}
...
preallocate_output(v::Vec{T,D}, x::AbstractArray{T,D}) where {T,D} = similar(x, T, (prod(v.m),))
...
function LinearAlgebra.mul!(y::AbstractArray{T,D}, v::invVec{T,D}, x::AbstractVector{T}) where {T,D}
    @assert size(y) == v.m
    fast_reshape!(y,x)
end
...
preallocate_input(v::IdentityOperator{TensorSpace{T,D}}) where {T,D} = Array{T,D}(undef, size(inputspace(v)))
...
preallocate_output(::RealPart{T}, x::AbstractArray{S,D}) where {T,D,S<:Union{Complex{T},T}} = Array{real(T),D}(undef, size(x))
