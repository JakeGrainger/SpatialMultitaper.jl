...
"""
function fft_anydomain(x::Array, grid::Grid, nfreq::NTuple{D,Int}, fmax::NTuple{D,Number}; kwargs...) where {D}
    embeddim(grid) ≤ ndims(x) || throw(ArgumentError("x must have at least $(embeddim(grid)) dimensions"))
    size(x)[1:embeddim(grid)] == size(grid) || throw(ArgumentError("x and grid must have the same size in overlapping dimensions (the first $(embeddim(grid))), but size of x is $(size(x)) and size of grid is $(size(grid))")) # need to improve this message
    
    # check that fmax is a integer multiple of nyquist
    nyquist = 1 ./ (2 .* unitless_spacing(grid))
    fmaxrel = round.(Int,fmax ./ nyquist)
    all(abs.(fmaxrel .- fmax./nyquist) .< 1e-10) || throw(ArgumentError("fmax must be a multiple of nyquist"))
    ...
    down_ind = if ndims(x) === embeddim(grid)
        ...
        CartesianIndices((freq_downsample_index.(nfreq, oversample)..., axes(x)[embeddim(grid)+1:ndims(x)]...))
    end
    ...
    J_unwrapped_reshaped = if ndims(J_unwrapped) === embeddim(grid)
        ...
        reshape(J_unwrapped, (size(J_unwrapped)[1:embeddim(grid)]...,prod(size(J_unwrapped)[embeddim(grid)+1:end])))
    end
