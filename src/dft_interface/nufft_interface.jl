...
function nufft1d1_anydomain!(output_storage, input_data, nfreq, fmax, iflag, eps; kwargs...)
    # unpack
    xj_rescaled, oversample_x, shift_x, cj =
        input_data.xj_rescaled, input_data.oversample_x, input_data.shift_x, input_data.cj
    oversampled_out, out, phase_correction =
        output_storage.oversampled_out, output_storage.out, output_storage.phase_correction
...
function nufft1d1_anydomain_precomp(interval, nfreq, fmax, xj, cj)
    ...
    oversampled_out =
        Array{complex(eltype(cj)),2}(undef, nfreq * oversample_x, n_transforms)
...
function nufft2d1_anydomain!(output_storage, input_data, nfreq, fmax, iflag, eps; kwargs...)
    # unpack
    xj_rescaled, oversample_x, shift_x, yj_rescaled, oversample_y, shift_y, cj =
        input_data.xj_rescaled,
    ...
    for i in axes(phase_correction, 2)
        phase_correction[:, i] .=
            exp.(exp_scaling .* (shift_x .* freq_x .+ (shift_y * freq_y[i])))
    end
