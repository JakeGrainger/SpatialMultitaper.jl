...
"""
Base.collect(estimate::AbstractEstimate) =
    tuple(getargument(estimate)..., getestimate(estimate))
Base.collect(estimate::IsotropicEstimate) =
    tuple(getargument(estimate), getestimate(estimate))
...
function _checkprocessbounds(::AbstractArray{SMatrix{P,P,T,L},D}, p, q) where {P,T,L,D}
    checkbounds(Bool, 1:P, p) || throw(
        BoundsError(
            "The first process index, $p, is out of bounds for estimate with size $((P,P))",
        ),
    )
    checkbounds(Bool, 1:P, q) || throw(
        BoundsError(
            "The second process index, $q, is out of bounds for estimate with size $((P,P))",
        ),
    )
end
...
function _checkindexbounds(
    estimate::AbstractArray{SMatrix{P,P,T,L},D},
    i::Vararg{Any,D},
) where {P,T,L,D}
