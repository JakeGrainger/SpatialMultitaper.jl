...
"""
function optimaltapers(region::Meshes.GeometryOrDomain, grid::CartesianGrid; freq_region::Meshes.GeometryOrDomain, ntapers::Int, freq_res, freq_downsample=nothing, real_tapers=true, tol=0.0)
    ...
    K = pixelate_region(fftfreq.(freq_res, 1 ./ downsample_spacing(grid,freq_downsample)), freq_region)
    h_oversize, λ = compute_eigenfunctions(R,K,ntapers; real_tapers=real_tapers, tol=tol)
    h = [prod(sqrt, unitless_spacing(grid)) .* reprocess(h_oversize[i], grid, freq_downsample) for i in eachindex(h_oversize)]
    return h, λ
end
...
function reprocess(h_large::AbstractArray{T,D}, grid, freq_downsample) where {D,T}
    return newweight(h_large,freq_downsample) .* upsample(h_large[ntuple(d->1:downsample_size(grid,freq_downsample)[d], Val{D}())...], grid, freq_downsample)
end
...
newweight(::AbstractArray{T,D}, freq_downsample::Real) where {T,D} = sqrt(1/freq_downsample)^D
newweight(::AbstractArray{T,D}, freq_downsample::NTuple{D,Real}) where {T,D} = prod(sqrt, inv.(freq_downsample))
...
downsample_spacing(grid::CartesianGrid, freq_downsample) = unitless_spacing(grid).*freq_downsample

pixelate_region(grid::CartesianGrid, shp::Meshes.GeometryOrDomain) = pixelate_region(grid2side(grid), shp)
function pixelate_region(g::NTuple{D,AbstractArray}, shp::Meshes.GeometryOrDomain) where {D}
