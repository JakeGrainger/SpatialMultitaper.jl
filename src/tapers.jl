...
"""
function L2_inner_product_interpolated(h::AbstractArray{T,D}, g::AbstractArray{T,D}, grid::CartesianGrid) where {T,D}
    ...
    prod(unitless_spacing(grid)) * sum(
        prod((-1)^(v[d]+v_prime[d])/3 + v[d]*v_prime[d] for d in 1:D) * sum(
           hx*gx for (hx,gx) in zip(view(g_padded, ntuple(d->v[d]+1:size(g,d), Val{D}())...), view(h_padded, ntuple(d->v[d]+1:size(g,d), Val{D}())...))
        )
...
function Interpolations.interpolate(taper::DiscreteTaper)
    ...
    scaled_taper = taper.taper.taper ./ sqrt(L2_inner_product_interpolated(taper.taper.taper, taper.taper.taper, taper.taper.grid))
    taper_interp = linear_interpolation(sides, scaled_taper, extrapolation_bc=zero(eltype(taper.taper.taper)))
    Taper(InterpolatedTaperFunc(taper_interp), InterpolatedTaperFT(taper.taper_ft, taper.taper.grid))
end
...
(f::InterpolatedTaperFT)(k::NTuple{D, Real}) where {D, Real} = f.taper_ft(k) * prod(sinc(π * unitless_spacing(f.grid)[d] * k[d])^2 for d in 1:D)
...
## show methods
Base.show(io::IO, ::MIME"text/plain", taper::ContinuousTaper) = print(io, "A continuous taper function.")
Base.show(io::IO, ::MIME"text/plain", taper::DiscreteTaper) = print(io, "A discrete taper function.")
Base.show(io::IO, ::MIME"text/plain", taper::InterpolatedTaper) = print(io, "A continuous interpolated taper function.")
Base.show(io::IO, ::MIME"text/plain", taper::TaperFamily) = print(io, "A family of $(length(taper)) taper functions.")
...
"""
function interpolated_taper_family(raw_tapers::Vector{<:AbstractArray}, grid::CartesianGrid; freq_res=size(grid))
    interpolated_tapers = TaperFamily(ntuple(d -> interpolate(discretetaper(raw_tapers[d], grid, freq_res=freq_res)), length(raw_tapers)))
    # checks orthogonality of interpolated tapers
    max_cross = maximum(abs(L2_inner_product_interpolated(raw_tapers[i], raw_tapers[j], grid)) for i in 1:length(interpolated_tapers) for j in i+1:length(interpolated_tapers))
    if max_cross > 1e-2
        max_discrete_cross = maximum(abs(sum(h*g for (h,g) in zip(raw_tapers[i], raw_tapers[j]))) for i in 1:length(interpolated_tapers) for j in i+1:length(interpolated_tapers))
...
function sin_taper_family(ntapers, region::Box)
    tapers = [make_sin_taper(d, region) for d in Iterators.ProductIterator(range.(1, ntapers))]
...
function sin_ft_base(k::Real, m::Int; tol=1e-10)
    ...
    elseif abs(2k + m) < tol
        ...
        return exp(-1im * π * (k - (m - 1) / 2)) / sqrt(2) * m * sin(π * k - π * m / 2) / (π * (k^2 - (m / 2)^2))
    end
end
