import SpatialMultitaper: Box, spectral_matrix, make_jk_weight, 
    tapered_dft, check_spatial_data, domain, values
@testset "api" begin
    @testset "check_spatial_data" begin
        ...
        multi_grid = georef((rf1=griddata.rf,rf2=rand(6*6)), CartesianGrid((0,0), (3,3), dims=(6,6)))

        @test_throws AssertionError SpatialMultitaper.check_spatial_data((pattern, griddata)) 
        @test_throws AssertionError SpatialMultitaper.check_spatial_data((domain(pattern), griddata)) 
        @test_warn "more than one random field provided to a geotable, currently we only process the first of these!" SpatialMultitaper.check_spatial_data((multi_grid,))
        
    end
    @testset "jk_weight" begin
        x = rand(30,4)
        @test spectral_matrix(x[[1:20; 22:30],:]) == spectral_matrix(x, make_jk_weight(30, 21))
    end
    @testset "apply_tapers" begin
        tapers = interpolated_taper_family([ones(5), ones(5)], CartesianGrid((-0.125,), (1.125,), dims=(5,)))
        X = georef((marks=ones(4),), PointSet([Point(0), Point(1), Point(0.5), Point(0.3)]))
        @test SpatialMultitaper.apply_taper(domain(X), values(X)[1], tapers) ≈ fill(complex(1), 8) # 8 == ntapers * npoints
    end
    @testset "pipeline" begin
        ...
        @testset "2d" begin
            ...
            @test mt_est.power[1,2,5,2] ≈ sum(dft[:,1,5,2] .* conj.(dft[:,2,5,2]))/3
        end
    end
    @testset "NaN handling" begin
        ...
        griddata = georef((rf=rand(6*6),), CartesianGrid((0,0), (3,3), dims=(6,6)))
        multi_grid = georef((rf1=griddata.rf,rf2=rand(6*6)), CartesianGrid((0,0), (3,3), dims=(6,6)))
        nan_grid = georef((rf=fill(NaN, 6*6),), CartesianGrid((0,0), (3,3), dims=(6,6)))
