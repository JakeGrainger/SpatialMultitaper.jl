import SpatialMultitaper: nufft_anydomain, nufft1d1_anydomain, nufft2d1_anydomain, nufft3d1_anydomain, 
    rescale_points, choose_freq_1d, freq_downsample_startindex, PointSet, Box, Point, box2sides, points2coords
...
function test_nufft3d1_anydomain(box, nfreq, fmax, xj, yj, zj, cj)
    ...
    slow_out = slow_dft(uj,cj,freq,-1)
    fast_out_pos = nufft3d1_anydomain(box, nfreq, fmax, xj, yj, zj, cj, 1, 1e-14)[:,:,:,1]
    slow_out_pos = slow_dft(uj,cj,freq,1)
    @test length(fast_out) == length(slow_out)
    @test fast_out ≈ slow_out
    @test fast_out_pos ≈ slow_out_pos
...
@testset "fft_interfaces" begin
    ...
    @testset "choose_freq_1d" begin
        nfreqs = [9,11,20]
        fmax = 0.5
        oversample = [1,2,3,7,21,22]
        @testset "nfreq=$nfreq, ovesample=$c" for nfreq in nfreqs, c in oversample
            @test choose_freq_1d(nfreq*c,fmax)[freq_downsample_startindex(nfreq,c):c:end] ≈ choose_freq_1d(nfreq,fmax)
        end
    end
    ...
    @testset "1d" begin
        ...
        @testset "fmax=$fmax, interval=$interval, nfreq=$nfreq" for fmax in fmaxs, interval in intervals, nfreq in nfreqs
    ...
    @testset "2d" begin
        # simple case
        intervals = [([-1/2,1/2],[-1/2,1/2]), ([-2.3,2.3],[-1.2,1.2]), ([-2.3, 1.4],[-0.6, 4.1])]
    ...
    @testset "3d" begin
        # simple case
        intervals = [([-1/2,1/2],[-1/2,1/2],[-1/2,1/2]), ([-2.3,2.3],[-1.2,1.2],[-6.4,6.4]), ([-2.3, 1.4],[-0.6, 4.1],[-0.8, 1.1])]
    ...
    @testset "box2sides" begin
        ...
        @test box2sides(Box(Point(0,3,-1), Point(4,5,-0.5))) == ((0,4), (3,5), (-1,-0.5))
    end
    ...
    @testset "general" begin
        ...
        nfreq = (4,4,4)
        fmax = (1,1,1)
        nufft_anydomain(Box(Point(0), Point(2)), nfreq[1:1], fmax[1:1], points1, cj, -1, 1e-9) == nufft1d1_anydomain((0,2), nfreq[1], fmax[1], [1,3], cj, -1, 1e-9)
        nufft_anydomain(Box(Point(0,1), Point(2,5)), nfreq[1:2], fmax[1:2], points2, cj, -1, 1e-9) == nufft2d1_anydomain(((0,2), (1,5)), nfreq[1:2], fmax[1:2], [1,3], [3,5], cj, -1, 1e-9)
        nufft_anydomain(Box(Point(0,1,2), Point(2,5,7)), nfreq, fmax, points3, cj, -1, 1e-9) == nufft3d1_anydomain(((0,2), (1,5), (2,7)), nfreq, fmax, [1,3], [3,5], [5,7], cj, -1, 1e-9)
    end
end