...
@testset "unwrap_fft_output_1d" begin # nyquist set to n/2 for integer values
    ...
    @testset "nfreq=$nfreq, fmaxrel=$fmaxrel" for nfreq in nfreqs, fmaxrel in fmaxrels
        x = Int.(Spmt.unwrap_fft_output(Spmt.choose_freq_1d(nfreq, nfreq / 2), (fmaxrel,)))
...
@testset "fft_anydomain" begin
    @testset "1d" begin
        @testset "basic" begin
            ...
            @test Spmt.fft_anydomain(y, grid, (8,), (1 / 2,)) ≈
                  Spmt.fftshift(Spmt.fft([1, 2, 3, 4, 0, 0, 0, 0])) # 8 is the smallest oversampling
            @test Spmt.fft_anydomain(y, grid, (8,), (1 / 2,)) ≈
                  slow_dft(0:3, y, Spmt.choose_freq_1d(8, 1 / 2), -1)

            @test Spmt.fft_anydomain(y_1_extra, grid, (4,), (1 / 2,)) ≈ hcat(
                [-2.0, -2.0 - 2.0im, 10.0, -2.0 + 2.0im],
                [-2.0, -2.0 - 2.0im, 10.0, -2.0 + 2.0im],
            )
        ...
        @testset "start=$start, nfreq=$nfreq, fmax=$fmax" for start in starts,
            ...
            for i in axes(Y_1_extra, 2)
                @test Y_1_extra[:, i] ≈
                      slow_dft(x, y_1_extra[:, i], Spmt.choose_freq_1d(nfreq, fmax), -1)
            end
            Y_2_extra = Spmt.fft_anydomain(y_2_extra, grid, (nfreq,), (fmax,))
            for i in axes(Y_2_extra, 2), j in axes(Y_2_extra, 3)
                @test Y_2_extra[:, i, j] ≈
                      slow_dft(x, y_2_extra[:, i, j], Spmt.choose_freq_1d(nfreq, fmax), -1)
            end
        end
    end
    ...
    @testset "2d" begin
        ...
        @testset "start=$start, nfreq=$nfreq, fmax=$fmax" for start in starts,
            ...
            x = collect(
                Iterators.product(start[1]:0.5:start[1]+1.75, start[2]:0.5:start[2]+3.75),
            )
            freq = Iterators.ProductIterator(Spmt.make_freq(nfreq, fmax, 2))
            @test Spmt.fft_anydomain(y, grid, nfreq, fmax) ≈ slow_dft(x, y, freq, -1)
            Y_1_extra = Spmt.fft_anydomain(y_1_extra, grid, nfreq, fmax)
