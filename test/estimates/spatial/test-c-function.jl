...
n_processes = 3
dim = 2
return_type = :vector  # test :single, :tuple, :vector
...
SpatialMultitaper.smoothed_rotational(
    (1:3, 1:3), randn(4, 4, 3, 3), [0.1, 0.2], SpatialMultitaper.RectKernel(10))
...
@testset "C-function tests" begin
    ...
    @testset "Dimension $dim tests" for dim in 1:3
        ...
        @testset "Raw data vs SpatialData consistency" begin
            ...
            points_spatial = make_points_example(rng, n_processes = 1, dim = dim,
                return_type = :single, point_number = 50)
            ...
            raw_result = c_function(
                points_raw[1], region, radii = radii, nfreq = nfreq, fmax = fmax, tapers = tapers)
        ...
        @testset "Return type: $return_type" for return_type in [:single, :tuple, :vector]
            ...
            @testset "C-function from SpatialData" begin
                ...
                @test all(isfinite, getestimate(result))
            end
            ...
            @testset "Partial c-function from SpatialData" begin
                if n_processes > 1  # Partial only makes sense for multiple processes
                    ...
                    @test_skip "Partial c-function requires multiple processes"
                end
            end
            ...
            @testset "Partial c-function from partial spectra" begin
                if n_processes > 1
                    ...
                    @test_skip "Partial spectra requires multiple processes"
                end
            end
            ...
            @testset "Partial c-function from spectra" begin
                if n_processes > 1
                    ...
                    @test_skip "Partial from spectra requires multiple processes"
                end
            end
            ...
            @testset "Indexing functionality" begin
                ...
                @test all(isfinite, values_result)
            end
        end
    end
    ...
    @testset "Edge cases and robustness" begin
        ...
        @test all(isfinite, getestimate(result_large))
    end
end
