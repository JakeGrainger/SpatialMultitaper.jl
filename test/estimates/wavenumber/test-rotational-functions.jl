...
import SpatialMultitaper: getestimates
...
@testset "rotational_coherence" begin
    ...
    @test all(isreal.(getestimates(result)))
    @test all(-1 .<= getestimates(result) .<= 1)
end
