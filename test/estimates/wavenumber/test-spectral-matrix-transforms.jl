...
import SpatialMultitaper: apply_transform, is_partial, Spectra, Coherence,
                          MultipleTupleTrait, MultipleVectorTrait
...
@testset "apply_transform function" begin
    @testset "SMatrix input" begin
        ...
        @testset "Function with no additional args" begin
            result = apply_transform(real, data, MultipleTupleTrait())
    ...
    @testset "Regular array input" begin
        ...
        @testset "Transform along matrix dimensions" begin
            ...
            result = apply_transform(det_transform, data, MultipleVectorTrait())
    ...
    @testset "Edge cases" begin
        ...
        result = apply_transform(x -> abs.(x), single_element, MultipleTupleTrait())
        @test length(result) == 1
        @test eltype(result) == SMatrix{1, 1, Float64, 1}

        # Empty arrays
        empty_data = SMatrix{2, 2, ComplexF64, 4}[]
        result = apply_transform(real, empty_data, MultipleTupleTrait())
        @test isempty(result)
    end
end
...
@testset "Type Preservation" begin
    ...
    @testset "Complex type handling" begin
        ...
        result_real = apply_transform(x -> real.(x), data, MultipleTupleTrait())
        @test eltype(eltype(result_real)) == Float32

        result_abs = apply_transform(x -> abs.(x), data, MultipleTupleTrait())
