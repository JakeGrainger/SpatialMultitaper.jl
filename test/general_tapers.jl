import SpatialMultitaper: downsample_spacing, reprocess
import SpatialMultitaper.Meshes: Ball, Point, discretize
@testset "general_tapers" begin
    ...
    @testset "optimaltapers" begin
        b = Box(Point(0,0), Point(1,1))
        g = CartesianGrid((0.,0.), (1.,1.), dims=(100,100))
        g2 = CartesianGrid((0.,0.), (1.,0.5), dims=(100,100))
        h = SpatialMultitaper.optimaltapers(b, g, freq_region=Ball(Point(0,0),0.1), ntapers=2, 
            freq_res=200, freq_downsample=2, tol=0.1)
        @test h[1] isa Vector{Matrix{Float64}}
        @test_throws AssertionError SpatialMultitaper.optimaltapers(b, g2, freq_region=Ball(Point(0,0),0.1), ntapers=2, 
            freq_res=200, freq_downsample=2, tol=0.1)
    end
end