import SpatialMultitaper: mean_estimate, Box
@testset "mean" begin
    ...
    @test mean_estimate(griddata, region, DefaultMean()) == 3.0 # (1+2+4+5)/4 (because of the region)
    @test mean_estimate(pointdata, points, region, KnownMean(1.0)) == 1.0
    @test mean_estimate(griddata, grid, region, KnownMean(1.0)) == 1.0
end