import SpatialMultitaper: grid2side, Taper, unitless_spacing
@testset "tapers" begin
    @testset "sin taper" begin
        ...
        @test tapers[6](0.4,3.4) ≈ SpatialMultitaper.sin_taper(0.4,3,10) * SpatialMultitaper.sin_taper(3.4,2,5)
        @test tapers[6](0.4,3.4) ≈ sin(π * 3 * 0.4 / 10) * sin(π * 2 * 3.4 / 5) * 2 / sqrt(50)
        @test tapers[6](0.4,3.4) ≈ tapers.tapers[6](0.4,3.4)

        # check quality of interpolation methods
    ...
    @testset "Taper" begin
        ...
        taper_family = interpolated_taper_family([[zeros(5,10); ones(5,10)], [ones(5,10); zeros(5,10)]], grid)        
