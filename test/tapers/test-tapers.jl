...
import SpatialMultitaper: sin_taper, L2_inner_product_interpolated, discretetaper,
                          DiscreteTaper, InterpolatedTaper, ContinuousTaper, taper_ft

@testset "sin taper" begin
    # check taper function is correct
    tapers = sin_taper_family((3, 3), Box(Point(0, 0), Point(10, 5)))
    @test tapers[6](0.4, 3.4) ≈ sin_taper(0.4, 3, 10) * sin_taper(3.4, 2, 5)
...
@testset "interpolation" begin
    ...
    @test L2_inner_product_interpolated(x, x, g) ≈ 99 + 2 / 3
end
@testset "Taper" begin
    grid = CartesianGrid((0, 0), (10, 10), dims = (10, 10))
    disc_taper = discretetaper(ones(10, 10), grid)
    @test disc_taper isa DiscreteTaper
