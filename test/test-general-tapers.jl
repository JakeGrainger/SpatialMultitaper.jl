...
@testset "pixelate_region" begin
	...
	@test Spmt.pixelate_region(g, b) == ones(Bool, 100, 100)
	@test Spmt.pixelate_region(g, discretize(b)) == ones(Bool, 100, 100)
	@test Spmt.pixelate_region(grid2side(g), b) == ones(Bool, 100, 100)
end
...
@testset "optimaltapers" begin
	...
	options = (freq_region = Ball(Point(0, 0), 0.1), ntapers = 2,
